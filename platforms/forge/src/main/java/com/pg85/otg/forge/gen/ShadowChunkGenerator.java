...
public class ShadowChunkGenerator
{
	...
	public void queueChunksForWorkerThreads(WorldGenRegion worldGenRegion, StructureFeatureManager manager, ChunkAccess chunk, ChunkGenerator chunkGenerator, OTGBiomeProvider biomeProvider, OTGChunkGenerator otgChunkGenerator, int worldHeightCap)
	{
		if(this.maxConcurrent > 0)
		{
			if(!this.threadsInitialized)
			{
				for(int i = 0; i < this.maxConcurrent; i++)
				{
					...
					Worker thread = this.new Worker(i, this.unloadedChunksCache, this.chunksToLoad, this.chunksBeingLoaded, worldGenRegion.getLevel(), chunkGenerator, biomeProvider, otgChunkGenerator, worldHeightCap);
	...
	private ForgeChunkBuffer getUnloadedChunk(OTGChunkGenerator otgChunkGenerator, Random random, ChunkCoordinate chunkCoordinate, ServerLevel level)
	{
	...
	public boolean checkHasVanillaStructureWithoutLoading(ServerLevel serverWorld, ChunkGenerator chunkGenerator, OTGBiomeProvider biomeProvider, ChunkCoordinate chunkCoordinate, ICachedBiomeProvider cachedBiomeProvider, boolean noiseAffectingStructuresOnly)
	{
		...
		if (serverWorld.getServer().getWorldData().worldGenSettings().generateFeatures())
		{
			...
			// TODO: Let users specify these ranges in world config? Also allow explicit range for a given modded structure
			...
			structuresPerDistance[4] = new ArrayList<String>(Arrays.asList(
				...
				"minecraft:monument",
				"minecraft:mansion"));
			structuresPerDistance[3] = new ArrayList<String>(List.of());
			structuresPerDistance[2] = new ArrayList<String>(List.of());
			...
			structuresPerDistance[0] = new ArrayList<String>(List.of());

			for(ChunkCoordinate chunkToHandle : chunksToHandle)
			{
				ChunkPos chunkpos = new ChunkPos(chunkToHandle.getChunkX(), chunkToHandle.getChunkZ());
				...
				IBiomeResourceLocation biomeLocation = biome.getBiomeConfig().getRegistryKey();

				var structureSetStream = chunkGenerator.possibleStructureSets();
				// We can get the registry name from ForgeRegistryEntry, which StructureFeature extends
				// We can ask if the structure spawns in a given biome with a predicate
				// We can see if the configured structure feature affects noise by checking its adaptnoise boolean
				// filter noise affecting only, if needed
				if (noiseAffectingStructuresOnly) {
					structureSetStream = structureSetStream.filter(set -> {
						if (set.unwrap().right().isEmpty()) return false;
						var entries = set.unwrap().right().get().structures();
						for (StructureSet.StructureSelectionEntry entry : entries) {
							if (entry.structure().value().adaptNoise)
								return true;
						}
						return false;
					});
				...
				// filter for biome
				structureSetStream = structureSetStream.filter(set -> {
					for (StructureSet.StructureSelectionEntry structure : set.value().structures()) {
						// Potentially slow; vanilla code uses set::contains as its predicate
						if (structure.generatesInMatchingBiome(biomeHolder -> {
							var registryName = biomeHolder.value().getRegistryName();
							return registryName != null && registryName.toString().equals(biomeLocation.toResourceLocationString());
						})) {
							return true;
						}
					}
				...
				// We now only have the structures to check for avoidance for. Loop through
				for (Holder<StructureSet> holder: structureSetStream.toList())
				{
					if (holder.unwrap().right().isEmpty())
					{
						return false;
					}
					var set = holder.unwrap().right().get();
					// Only do this once, instead of for each structure in the set
					if (!set.placement().isFeatureChunk(chunkGenerator, serverWorld.getSeed(), chunkpos.x, chunkpos.z))
					{
						// We don't have a placement here for this set, so skip this set
						continue;
					}

					for (StructureSet.StructureSelectionEntry entry : set.structures())
					{
						var registryName = entry.structure().value().feature.getRegistryName();
						if (registryName == null)
						{
							return false;
						}
						var searchDistance = 0;
						// Use default search distance of 1 for modded structures
						if (!registryName.getNamespace().equals("minecraft"))
						{
							searchDistance = 1;
						} else {
							for (int i = 0; i < structuresPerDistance.length; i++)
							{
								if (structuresPerDistance[i].contains(registryName.toString()))
								{
									searchDistance = i;
									break;
								}
						...
						// We already know there is a structure here, because check was moved out of the for loop
						chunksHandled.put(chunkToHandle, searchDistance);
				...
				// Add everything else with a distance of 0
	...
	public ForgeChunkBuffer getChunkWithoutLoadingOrCaching(OTGChunkGenerator otgChunkGenerator, Random random, ChunkCoordinate chunkCoordinate, ServerLevel level)
	{
	...
	private class Worker implements Runnable
	{
		...
		Worker(int index, FifoMap<ChunkCoordinate, ChunkAccess> unloadedChunksCache, List<ChunkCoordinate> chunksToLoad, ChunkCoordinate[] chunksBeingLoaded, ServerLevel serverWorld, ChunkGenerator chunkGenerator, OTGBiomeProvider biomeProvider, OTGChunkGenerator otgChunkGenerator, int worldHeightCap)
		{
			this.index = index;
		...
		public void run()
		{
			...
			while(true)
			{
				...
				if(coords != null)
				{
					if(!checkHasVanillaStructureWithoutLoading(this.serverWorld, this.chunkGenerator, this.biomeProvider, coords, this.otgChunkGenerator.getCachedBiomeProvider(), true))
					{
						// Generate a chunk without loading/decorating it.
						ChunkAccess cachedChunk = getUnloadedChunk(this.otgChunkGenerator, this.worldRandom, coords, this.serverWorld.getLevel()).getChunk();
						synchronized(workerLock)
						{
